        ...
        for i, batch in enumerate(val_loader):
            ...
            if cfg.TPT.EATA_TYPE == 'top5':
                if tta_pred in base_top5:
                    group_models[group_idx].prompt_learner = update_ema_variables(group_models[group_idx].prompt_learner, model.prompt_learner, cfg.TPT.EMA)
            ...
            if (i + 1) % cfg.TPT.RESET_STEPS == 0:
                with torch.no_grad():
                    ...
                    for g in range(n_groups):
                        ...
                        group_meters_tta.append(AverageMeter_TPT(f'Group{i}_Acc', ':6.2f', Summary.AVERAGE))
                        ema_update_idx[g] = []
                        group_models[g].reset()
                print(f'Resetting all models  and group meters at step {i}')
    ...
    def test_time_tuning(self, model, inputs, optimizer, scaler, args):
        ...
        for j in range(args.TTA_STEPS):
            with torch.cuda.amp.autocast():
                ...
                loss_orth = sim[~torch.eye(*sim.shape,dtype = torch.bool)].mean() * 0.5
                loss += loss_orth


                print(loss_ent.item(), loss_align.item(), loss_orth.item())
